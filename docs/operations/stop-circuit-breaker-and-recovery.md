...
用途：终止一个具体 task attempt，不影响同项目其他任务。
...
1. Task Engine 将状态改为 `STOPPING`；
2. Executor 停止接收该任务的新子步骤；
3. 向进程组发送协作式取消信号；
4. 刷新 stdout、stderr、事件和游标；
5. 等待宽限期；
6. 未退出则发送强制终止信号；
7. 记录最终退出原因；
8. 状态进入 `CANCELLED`，而不是 `FAILED` 或 `SUCCEEDED`。

### 2.2 Project Pause

用途：暂停一个项目的新任务，并根据策略处理当前任务。
...
- `drain`：不接收新任务，允许当前任务结束；
- `stop`：不接收新任务，并停止当前可取消任务。

暂停不等于删除注册信息。Watcher 默认继续只读观察，以便查看暂停期间的外部变化。

### 2.3 Controller Drain

用途：控制器维护、升级或关机前排空所有任务。
...
- 拒绝新任务；
- 保持 API 查询和 watcher 工作；
- 等待活动任务完成；
- 到达 drain deadline 后按项目策略停止剩余任务；
- 完成数据库 checkpoint 和证据 flush；
- 标记为 `DRAINED`。

### 2.4 Emergency Stop

用途：存在凭据泄漏、误执行、磁盘故障、进程失控或用户明确要求立即停止时使用。
...
哨兵文件由控制器写入，人工创建时会被控制器导入为带 actor 的控制事件。
...
资源熔断优先阻止新任务，尽量不直接杀死正在安全运行的业务任务；只有资源继续恶化才升级为 stop。
...
- Task Stop：允许发布最终取消报告；
- Project Pause：允许更新项目状态摘要；
- Controller Drain：允许完成已排队的安全报告同步；
...
1. 单任务停止不会误伤同项目其他任务。
2. 项目 `drain` 不接收新任务，但允许当前任务完成。
3. Emergency Stop 能在限定时间内阻止新的执行和 SCM 写入。
4. 强制终止前至少尝试一次证据 flush。
5. 控制器重启后不会自动重跑未知副作用任务。
6. orphan 任务被明确标记，不伪装为成功或失败。
7. 所有 stop、resume、clear 操作均带 actor、reason 和时间戳。
8. 重复停止请求具有幂等性。
9. shared/external 资源不会被普通 task stop 错误关闭。
10. 所有停止路径均有自动化测试和故障注入测试。
