...
这些版本不得被一个单一版本号隐式替代。应用升级时必须声明支持范围。
...
Smoke test 未通过时不恢复 SCM 写入和自动调度。
